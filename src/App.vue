...
function renderItem(
  ...
  if (item.type === 'text' && item.text && item.position) {
    ...
    const cols = item.repeat.columns === 'infinite'
      ? Math.ceil(canvasWidth.value / width) + 1
      : (item.repeat.columns ? Number(item.repeat.columns) : 1);
    ...
    if (typeof item === 'string') {
      ...
      if (['M', 'L', 'Z', 'H', 'V', 'C', 'S', 'Q', 'T', 'A'].includes(command)) {
        ...
        if (command === 'Z') {
          ...
          if (i < data.length) {
            const val = evaluateExpression(data[i], vars);
